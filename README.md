...
Один приём держит весь сайт: **у всего, что можно нажать или что лежит поверх
фона, обводка в два пикселя чернилами и жёсткая тень со смещением** — без
размытия, как у переводной картинки. Радиус при этом кодирует роль: карточка
28, плитка 22, кнопка 14, поле 12, пилюля 999. По одному скруглению видно,
что перед тобой.
...
Текст при этом **всегда чернильный** `#2A1E4A`. Это не осторожность: так у всех
девяти пар одни и те же 4,5:1, и ни одна не читается хуже соседней. Измерено —
самая слабая пара даёт 10,85:1, пиктограмма в своём кружке — 5,39:1, текст
кнопки на коралле — 4,90:1.

Интерфейсных цветов три, и у каждого одна работа: **коралл** — действие,
**бирюза** — окно в расписании, **золото** — время поджимает (занятие вылезло
за окно, старт в ближайшие две недели) и закладка «сохранить поиск». «Набор
идёт» к этому не относится и остаётся нейтральным. Возраст ±1 — мягкая
граница, где решает звонок в центр, поэтому она показана полым маркером
и пунктирной карточкой, а не третьим оттенком.

Палитра лежит в одном месте — в `css/app.css` под `[data-dir="…"]`. Разметка
её не знает: SVG берёт `var(--d-chip)` и `var(--d-wash)`, пиктограммы —
`currentColor`. Рассинхронизировать цвет между поиском и статикой нечем.

**Движение на сайте ровно одно:** нажатие вдавливает элемент на два пикселя
в тень и укорачивает её. Оно отвечает на действие человека, а не происходит
само; при `prefers-reduced-motion` выключается вместе со всем остальным.
Ничего не выезжает при прокрутке.

**Пиктограммы** нарисованы под одно ограничение: сетка 24×24, обводка 2,
только горизонтали, вертикали, диагонали под 45° и окружности. Дуга на девять
знаков ровно одна — у танцев, где она значит движение. Каждая лежит в кружке
своего направления — это единственное место, где у направления появляется
полный цвет.
...
середины рёбер: выходит замкнутая клякса без единой прямой. Число пятен
выведено из пропорции, поэтому одна и та же обложка читается и на полосе
карточки 88×240, и на ленте страницы группы 960×120. Фотографий детей нет
нигде.

**Подвал** отделён от страницы не линейкой, а фестонами: повтор одного эллипса
цвета бумаги поверх тёмной плашки. Ни картинки, ни лишней разметки это
не требует. В подвале девять ссылок на направления — единственная
перелинковка, которая есть на каждой странице; собирается из каталога, чтобы
после смены слага ссылка не уводила в 404.
