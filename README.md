...
**Обложки** карточек строятся из id группы: у 310 групп 310 разных обложек,
и при пересборке каталога они не перетасовываются. Рисунок — несколько мягких
пятен, наползающих друг на друга и обрезанных краем. Пятно строится по кольцу
точек со смещёнными радиусами и замыкается квадратичными кривыми через
середины рёбер. Число пятен выведено из пропорции, поэтому одна и та же
обложка читается и полосой 240×150 в карточке, и лентой 960×120 на странице
группы. Фотографий детей нет нигде.
...
Шрифтов два и они предельно разные: **Unbounded** 700–800 в заголовках
и числах, **Nunito** 400–900 в тексте, чипах и кнопках. Всё остальное —
...
**Трёх колонок в результатах — их две.** В эталоне карточка это заглушка:
`[Название группы]`, `[ЦЕНА]`, три чипа. В настоящей карточке живёт полоса
занятия на шкале дня — главная графика проекта, — и в колонке 300 px она
нечитаема. В двух колонках на полосу остаётся 330 px, и видно, на сколько
минут занятие вылезает за окно.
