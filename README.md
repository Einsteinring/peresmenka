...
Telegram Login Widget, без паролей и почты. Подпись проверяется на сервере:
...
Сессия — httpOnly-кука `ps_session` на 90 дней, `SameSite=Lax`, `Secure` на
боевом домене. В виджете стоит `data-request-access="write"`: без него бот
не сможет написать первым, и уведомления в Telegram работать не будут.
...
67 тестов. Из них по кабинету: девять на подпись (правильная проходит,
подделка любого поля не проходит, чужой токен не проходит, протухший
и будущий `auth_date` не проходят), четыре на тихие часы с границами,
пять на имена поисков и одиннадцать на владение объектами.
...
3. Имя бота без `@` положить в `TELEGRAM_BOT_USERNAME`: его отдаёт `/api/me`,
   и по нему страница собирает виджет.
4. В Upstash создать базу и взять REST URL и токен.
5. Придумать `CRON_SECRET`: без него `/api/cron/match` отвечает 401.
...
api/_lib/               сессия, Upstash, Telegram, ответы — общее
